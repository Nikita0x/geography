...
class FlagsAsiaScreen extends StatefulWidget {
  const FlagsAsiaScreen({super.key});

  @override
  State<FlagsAsiaScreen> createState() => _FlagsAsiaScreenState();
}

class _FlagsAsiaScreenState extends State<FlagsAsiaScreen> {
  int duration = 60;
  ...
  void generateNewRound() {
    ...
    final correctCountry =
        asianCountries[random.nextInt(asianCountries.length)];

    final incorrectCountries =
        (List<Country>.from(asianCountries)
  ...
  Widget build(BuildContext context) {
    return CupertinoPageScaffold(
      navigationBar: const CupertinoNavigationBar(middle: Text('Asia')),
      child: SafeArea(
        child:
            duration == 0
                ? Center(
