...
class FlagsAfricaScreen extends StatefulWidget {
  const FlagsAfricaScreen({super.key});

  @override
  State<FlagsAfricaScreen> createState() => _FlagsAfricaScreenState();
}

class _FlagsAfricaScreenState extends State<FlagsAfricaScreen> {
  int duration = 60;
  ...
  void generateNewRound() {
    ...
    final correctCountry =
        africanCountries[random.nextInt(africanCountries.length)];

    final incorrectCountries =
        (List<Country>.from(africanCountries)
  ...
  Widget build(BuildContext context) {
    return CupertinoPageScaffold(
      navigationBar: const CupertinoNavigationBar(middle: Text('Africa')),
      child: SafeArea(
        child:
            duration == 0
                ? Center(
